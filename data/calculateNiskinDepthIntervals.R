...
# remove outliers
niskin_raw_filtdep <- niskin_raw %>% 
  #filter(depth <= 100) %>%
  filter(Chlorophyll <= 20 | is.na(Chlorophyll))  # Preserve NAs for other variables
...
niskin_int_0 <- getNiskinInterpCounts(depth_from=0, depth_to=24, noofNA=5)
niskin_int_1 <- getNiskinInterpCounts(depth_from=25, depth_to=49, noofNA=5)
niskin_int_2 <- getNiskinInterpCounts(depth_from=50, depth_to=74, noofNA=5)
...
niskin_dat$depth <- factor(niskin_dat$depth, levels=c("75-100","50-74","25-49", "0-24"))
levels(niskin_dat$depth) <- c("75-100","50-75","25-50", "0-25")
